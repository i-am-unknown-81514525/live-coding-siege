...
) -> Callable[Concatenate[C, list[str], P], T | None]:
    @filter_allowed
    def inner(ctx: C, groups: list[str], *args: P.args, **kwargs: P.kwargs) -> T | None:
        if not groups:
            group_names = ", ".join(f'"{name}"' for name in groups)
            ctx.private_send(
                text=f'Missing required group of "Allowed" or "Authorised" in one of {group_names}'
            )
            return None
...
def require_authorised[**P, T, C: Context](
    func: Callable[Concatenate[C, list[str], P], T],
) -> Callable[Concatenate[C, list[str], P], T | None]:
    @filter_authorised
    def inner(ctx: C, groups: list[str], *args: P.args, **kwargs: P.kwargs) -> T | None:
        if not groups:
            group_names = ", ".join(f'"{name}"' for name in groups)
            ctx.private_send(
                text=f'Missing required group of "Authorised" in one of {group_names}'
            )
            return None
        return func(ctx, groups, *args, **kwargs)
...
def filter_allowed[**P, T, C: Context](
    func: Callable[Concatenate[C, list[str], P], T],
...
def filter_authorised[**P, T, C: Context](
    func: Callable[Concatenate[C, list[str], P], T],
) -> Callable[Concatenate[C, list[str], P], T]:
    def inner(ctx: C, groups: list[str], *args: P.args, **kwargs: P.kwargs) -> T:
        configs = get_config()["bot"]["group"]
...
def filter_groups[**P, T, C: Context](
    groups: list[str],
) -> Callable[
    [Callable[Concatenate[C, list[str], P], T]],
    Callable[Concatenate[C, list[str], P], T],
    ...
    def outer(
        ...
        def inner(
            ctx: C, curr_group: list[str], *args: P.args, **kwargs: P.kwargs
        ) -> T:
            return func(
                ctx, list(set(groups).intersection(curr_group)), *args, **kwargs
            )
...
@overload
def require_group[**P, T, C: Context]( # pyright: ignore[reportOverlappingOverload]
    group: str, forwarding: Literal[False] = False
) -> Callable[
    [Callable[Concatenate[C, P], T]], Callable[Concatenate[C, list[str], P], T | None]
]: ... 


@overload
def require_group[**P, T, C: Context](
    group: str, forwarding: Literal[True] = False  # pyright: ignore[reportArgumentType]
) -> Callable[
    [Callable[Concatenate[C, list[str], P], T]],
    Callable[Concatenate[C, list[str], P], T | None],
]: ... 
...
def require_group[**P, T, C: Context](
    group: str, forwarding: bool = False
) -> (
    Callable[
        [Callable[Concatenate[C, list[str], P], T]],
        Callable[Concatenate[C, list[str], P], T | None],
    ...
    def outer_fwd(
        ...
        def inner(
            ctx: C, groups: list[str], *args: P.args, **kwargs: P.kwargs
        ) -> T | None:
...
@overload
def require_groups[**P, T, C: Context](  # pyright: ignore[reportOverlappingOverload]
    groups: list[str], forwarding: Literal[False] = False  
) -> Callable[
    [Callable[Concatenate[C, P], T]], Callable[Concatenate[C, list[str], P], T | None]
]: ...
...
@overload
def require_groups[**P, T, C: Context]( 
    groups: list[str], forwarding: Literal[True] = False   # pyright: ignore[reportArgumentType]
) -> Callable[
    [Callable[Concatenate[C, list[str], P], T]],
    Callable[Concatenate[C, list[str], P], T | None],
]: ...
...
def require_groups[**P, T, C: Context](
    groups: list[str], forwarding: bool = False
) -> (
    Callable[
        [Callable[Concatenate[C, list[str], P], T]],
        Callable[Concatenate[C, list[str], P], T | None],
    ...
    def outer_fwd(
        ...
        def inner(
            ctx: C, inner_group: list[str], *args: P.args, **kwargs: P.kwargs
        ) -> T | None:
...
@overload
def has_group[**P, T, C: Context](  # pyright: ignore[reportOverlappingOverload]
    group: str, forwarding: Literal[False] = False
) -> Callable[
...
@overload
def has_group[**P, T, C: Context](
    group: str, forwarding: Literal[True] = False  # pyright: ignore[reportArgumentType]
) -> Callable[
    [Callable[Concatenate[C, bool, list[str], P], T]],
    Callable[Concatenate[C, list[str], P], T | None],
]: ... 
...
def has_group[**P, T, C: Context](
    ...
    def outer_fwd(
        func: Callable[Concatenate[C, bool, list[str], P], T],
    ) -> Callable[Concatenate[C, list[str], P], T | None]:
        def inner(
            ctx: C, groups: list[str], *args: P.args, **kwargs: P.kwargs
        ) -> T | None:
...
@overload
def have_one_of_groups[**P, T, C: Context](  # pyright: ignore[reportOverlappingOverload]
    groups: list[str], forwarding: Literal[False] = False
) -> Callable[
    [Callable[Concatenate[C, bool, P], T]],
    Callable[Concatenate[C, list[str], P], T | None],
...
@overload
def have_one_of_groups[**P, T, C: Context](
    groups: list[str], forwarding: Literal[True] = False  # pyright: ignore[reportArgumentType]
) -> Callable[
    [Callable[Concatenate[C, bool, list[str], P], T]],
    Callable[Concatenate[C, list[str], P], T | None],
...
@overload
def have_any_group[**P, T, C: Context](  # pyright: ignore[reportOverlappingOverload]
    forwarding: Literal[False] = False,
) -> Callable[
    [Callable[Concatenate[C, bool, P], T]],
    Callable[Concatenate[C, list[str], P], T | None],
]: ... 
...
@overload
def have_any_group[**P, T, C: Context](
    forwarding: Literal[True] = False,  # pyright: ignore[reportArgumentType]
) -> Callable[
    [Callable[Concatenate[C, bool, list[str], P], T]],
    Callable[Concatenate[C, list[str], P], T | None],
]: ... 
...
def consume_second[**P, A, B, T](
    func: Callable[Concatenate[A, P], T],
) -> Callable[Concatenate[A, B, P], T]:  # pyright: ignore[reportInvalidTypeVarUse]
...
def duplicate_second[**P, A, B, T](
    func: Callable[Concatenate[A, B, B, P], T],
) -> Callable[Concatenate[A, B, P], T]:
...
def swap[**P, A, B, C, T](
    func: Callable[Concatenate[A, C, B, P], T],
) -> Callable[Concatenate[A, B, C, P], T]:
