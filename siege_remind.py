...
def remind_loop(client: BaseSocketModeClient):
    while True:
        ...
        try:
            ...
            mention: bool = True
            try: 
                profile = UserProfile.parse(client.web_client.users_profile_get(user=slack_id).data["profile"]) # type: ignore
            ...
            latest_heartbeat = max(max_week_heartbeats, key=lambda x: x.measurement_time.timestamp())
            one_h_ago_heartbeat = max(filter(lambda x: x.measurement_time.timestamp() <= time.time() - 3600, max_week_heartbeats), default=None, key=lambda x: x.measurement_time.timestamp())
            if latest_heartbeat.hours - (one_h_ago_heartbeat.hours if one_h_ago_heartbeat else 0) < 0.09:    
                send_remind(
                    client=client,
                    user=user,
                    channel=os.environ["REMIND_CHANNEL"],
